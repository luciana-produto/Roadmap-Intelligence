...
public interface IRoadmapDemandRepository : IRepository<RoadmapDemand>
{
    ...
    Task<IReadOnlyList<RoadmapDemand>> GetByIdsForUpdateAsync(IEnumerable<Guid> ids, CancellationToken cancellationToken = default);
    ...
    Task RemoveDependenciesPointingToAsync(
        Guid blockerDemandId,
        IEnumerable<Guid> dependentDemandIds,
        CancellationToken cancellationToken = default);

    Task RemoveAllDependenciesInvolvingAsync(
        Guid demandId,
        CancellationToken cancellationToken = default);
